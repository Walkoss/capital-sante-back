...
namespace AuthController {
    ...
    export async function login(req: Request, res: Response, next: Function) {
        try {
            ...
            if (!user) {
                res.status(httpStatus.UNAUTHORIZED).send({success: false, message: 'Authentication failed. User not found.'});
            } else {
                user.authenticate(req.body.password, (err, isMatch) => {
                    if (isMatch && !err) {
                        ...
                        res.status(httpStatus.UNAUTHORIZED).send({success: false, message: 'Authentication failed. Passwords did not match.'});
                    }
                });
            }
        } catch (error) {
